...
const int MAX_LEVEL = 10;
    ...
    // Calculates HP/MP/Attack/Defense in one place so formulas aren't repeated in multiple places
    void calcCombatStats(int classChoice, int level, int str, int intl, int dex, int con,
                     int &hp, int &mp, double &atk, int &def) {
    // Base formulas
    hp = (con * 10) + (level * 5);
    mp = (intl * 10) + (level * 3);
    ...
    if (classChoice == 1) {                              // Warrior
        hp += 20;
    } else if (classChoice == 2) {                      // Mage
        mp += 30;
    }

    // Attack Power by class
    switch (classChoice) {
...
int main() {
    ...
    // Keep base stats for display, but use modified stats for calculations
    ...
    do {
        ...
        switch (choice) {
            ...
            case 2: {
                // VIEW STATS
                // Blocks option until character is created
                if (!characterCreated) {
            ...
            case 3: {
                ...
                calcCombatStats(classChoice, level, str, intl, dex, con, hp, mp, atk, def);
            ...
            case 5:
                cout << "\nThank you for using GameStat! Goodbye!\n";
                keepRunning = false;
                break;
        }
